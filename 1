/*
	$Id: giftblaster.js,v 1.34 2012-06-26 05:14:09 eike Exp $
...
javascript:(function (){
	...
	option='',
	single = false,
	rev=/,v \d+\.(\d+)\s201/.exec("$Id: giftblaster.js,v 1.34 2012-06-26 05:14:09 eike Exp $")[1],
	version = '  Gift-Blaster v1.'+rev+' beta';
	...
	var allCats = {"487":8,"503":1,"523":1,"100":1,"504":1,"189":1,"422":1,"493":4,"417":1,"418":1,"438":1,"1005":1,"458":1,"456":1,"457":1};
		...
		freegifts['Red Mystery Bag'] = new giftObject('Red Mystery Bag',-5,-5,'#ff0000');
		freegifts['Be-Leavers'] = new giftObject('jammys*',1108,1,'#FFA500');
		...
		// WD
		freegifts['ArcWelder'] = new giftObject('ArcWelder*',75,1,'#CC00FF');
		freegifts['Buzzsaw'] = new giftObject('Buzzsaw*',76,1,'#CC00FF');
		freegifts['GunPowder'] = new giftObject('GunPowder*',77,1,'#CC00FF');
		freegifts['Gundrill'] = new giftObject('Gundrill*',78,1,'#CC00FF');
		freegifts['Forge'] = new giftObject('Forge*',79,1,'#CC00FF');
		...
		//London
		//freegifts['London Invitation'] = new giftObject('London Invitation',528,1,'#E08A03');
		freegifts['Vintage Lighter'] = new giftObject('Vintage Lighter*',529,1,'#E08A03');
		freegifts['London Brick'] = new giftObject('London Brick*',1015,1,'#E08A03');
		freegifts['London Tile'] = new giftObject('London Tile*',1016,1,'#E08A03');
		freegifts['London Hammer'] = new giftObject('London Hammer*',1017,1,'#E08A03');
		freegifts['London Plumb'] = new giftObject('London Plumb*',1018,1,'#E08A03');
		freegifts['London Girder'] = new giftObject('London Girder*',1019,1,'#E08A03');
		
		freegifts['Brazil Crew'] = new giftObject('Brazil/Chicago Crew',-2,-2,'#ff0000');
		freegifts['Chicago Crew'] = new giftObject('Brazil/Chicago Crew',-6,-6,'#ff0000');
		...
		if($('#quest_tray_inner .task_buttons span:contains("Get Help")').length>0) {
			freegifts['Mission Crew Help'] = new giftObject('Mission Crew Help',-1,-1,'#ff0000');
		}
	...
	var ids = [];
	var playerlist = {}, playerlist_autocomplete = {} ;
	var allFriends=[];
	var logs=[];
	var run=false;
	var total_ids=0;
	...
	var style = '<style type="text/css">'+
		...
		'#'+spocklet+'_loadbyname { cursor:pointer; color:#FFD927;}'+
	'</style>';
	...
	'<div id="'+spocklet+'_main">'+
		...
		'<tr><td colspan="3">'+
		'Select Gift: <input style="display:'+(debug?'':'none')+';" id="'+spocklet+'_giftid" type="text" value="400" /><span id="'+spocklet+'_option">loading gifts...</span><br />'+
		'<P>Number of different IDs (max 50): <INPUT TYPE=text NAME="amount" id="txtamount" Value="1"><BR><BR>'+		
		'Enter IDs here or click a button to fill them. [<span id="'+spocklet+'_numids">0</span>] [<a href="#" id="'+spocklet+'_clearlist">clear list</a>] :<br /><textarea cols=90 rows=8 id="'+spocklet+'_list"></textarea><br />'+
		...
		'<br />'+
		'<span id="'+spocklet+'_loadbyname">Select people by name</span></span><span id="'+spocklet+'_byname" style="display:none;">Add by name: <input type=text id="'+spocklet+'_autocomplete" style="width:200px;" /> '+
		'<a href="#" id="'+spocklet+'_mafia" class="sexy_button_new short green" title="Add" alt="Add"><span><span>Add</span></span></a></span> '+
		...
		'</td></tr>'+
		'<tr><td colspan="3">Progress: <br /><div id="'+spocklet+'_progressbar"></div></td></tr>'+
	...
	$('#'+spocklet+'_clearlist').click(function() {
		ids=[];
		$('#'+spocklet+'_list').val(ids.join(','));
		$('#'+spocklet+'_numids').text(ids.length);
		return false;
	});
	$('#'+spocklet+'_loadbyname').click(function() {
		load_mafia_ids(false);
		$('#'+spocklet+'_loadbyname').hide();
		return false;
	});
...
// get MW ids and put them into the list
	function load_mafia_ids(display){
	ids=[]; playerlist = {}; playerlist_autocomplete = [];
			...
			if (Util.isset(response.authResponse)) {
				...
				FB.api('/me/friends?access_token='+access_token,function(response){
					for (x in response.data) {
						...
						playerlist[response.data[x].name]=response.data[x].id;
						playerlist_autocomplete.push(response.data[x].name);
					}
					if(!display){
						$('#'+spocklet+'_byname').show();
						$('#'+spocklet+'_autocomplete').autocomplete({
							source: playerlist_autocomplete,
							select:function(event, ui){
								if(playerlist[ui.item.value]) {
									ids.push(playerlist[ui.item.value]);
									$('#'+spocklet+'_list').val(ids.join(','));
									$('#'+spocklet+'_numids').text(ids.length);
									setTimeout(function(){$('#'+spocklet+'_autocomplete').val('');},1000);
								}
							}
						});
						ids=[];
					} else {
						log('Facebook Friends Loaded');
					}
					$('#'+spocklet+'_list').val(ids.join(','));
					$('#'+spocklet+'_numids').text(ids.length);
	...
	function start_sending(){
		var idstring = $('#'+spocklet+'_list').val();
		if(idstring.indexOf('$')==0) { idstring=idstring.substr(1); single = true; }
		ids=idstring.split(',');
		...
		run=true;
		process_all();
		setTimeout(process_all,200);
		setTimeout(process_all,400);
		setTimeout(process_all,600);
		setTimeout(process_all,800);
		setTimeout(process_all,1000);
		setTimeout(process_all,1200);
		setTimeout(process_all,1400);
		setTimeout(process_all,1600);
	}
	...
	function process_all(){
		if(ids.length>0 && run && (explist.length>0)){
			...
			if(num>parseInt(document.getElementById('txtamount').value)) { num=parseInt(document.getElementById('txtamount').value); }
			if(single){ num=1;}
			explist[0].num-=num;
	...
	function getParams_CityCrew(callback,city){
		request('xw_controller=requests&xw_action=pop_mfs&xw_city='+city+'&tmp=&cb=&xw_person='+User.id.substr(2)+'&mwcom=1&request_id=city_crew_brazil&xw_client_id=8',function(msg){
			if(m=/MW.Request.setData\(\'([^\']*)\'/.exec(msg)){
				$('#'+spocklet+'_params').val(m[1]);
				callback();
			}
...
//		MW.Request.setData('{"from_user":"p|95497006","cckey":"1b485f1f798d035cd967737ef7ae117d","time_id":"1323333909","city_id":"8","type":1,"time":1323333909,"hash":"a383463df7609012db9f23514c659c5d"}');
	...
	function getParams_rmb(callback){
		request('xw_controller=requests&xw_action=friend_selector&xw_city=1&tmp=&cb=&xw_person='+User.id.substr(2)+'&req_controller=job&xw_client_id=8&ajax=1&liteload=1&fbml_iframe=1',function(msg){
			if(m=/MW.Request.setData\(\'([^\']*)\'/.exec(msg)){
				$('#'+spocklet+'_params').val(m[1]);
				callback();
			}
...
//		MW.Request.setData('{"from_user":"p|95497006","cckey":"1b485f1f798d035cd967737ef7ae117d","time_id":"1323333909","city_id":"8","type":1,"time":1323333909,"hash":"a383463df7609012db9f23514c659c5d"}');
	...
	function getParams(callback){
		...
		if ((id==-1) && (cat==-1)) {
			...
			// city crew link
			getParams_CityCrew(callback,7);
		} else if ((id==-6) && (cat==-6)) {
			// city crew link
			getParams_CityCrew(callback,8);
		} else if ((id==-3) && (cat==-3)) {
			// energy pack
...
//			MW.Request.setData('{"from_user":"p|95497006","from_user_ppid":"p|95497006","time_id":"1322638872","loop":"c83555e443934fe312565ee53ae7c9da","item_cat":"1","item_id":"527","gkey":"e751c0bc1709b9d948c307e16a46e99e","giftback_key":"4130cfb5edeee8ddee28b4f8bc1f164d","mult":"1","activehustle":"send_gifts","over_9000":"true","exclude_type":"%EXCLUDE_TYPE%","simple_key":"11308216451322638872","type":4,"time":1322638872,"hash":"c12652ddf63dfc927140b885aa99a7d4"}');
	...
	function log(message){
		var newmessage='<span class="more_in">'+timestamp()+'</span> '+message;
		var limit = 20;
		...
		$('#'+spocklet+'_status').html(message);
	}	
	
	// delay "seconds" seconds, then proceed to handler()
	function delay(seconds,handler) {
		seconds=parseInt(seconds);
		if(seconds>0) {
			$('#'+spocklet+'_status').html('Sending next gifts in '+seconds+'s...');
			seconds--;
			setTimeout(delay,1000,seconds,handler);
		} else {
			handler();
		}
	}
